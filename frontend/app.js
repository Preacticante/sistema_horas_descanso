...
async function enviarRegistroHoras(event) {
    ...
    if (!motivo || motivo.length < 5) {
        mostrarNotificacion('warning', 'Motivo requerido', 'El motivo debe tener al menos 5 caracteres.');
        return;
    }

    if (idJefeDirectoRaw && (idJefeDirecto === null || Number.isNaN(idJefeDirecto) || idJefeDirecto <= 0)) {
        mostrarNotificacion('warning', 'ID inválido', 'El ID de jefe directo no es válido.');
        return;
    }

    if (idJefeSuperiorRaw && (idJefeSuperior === null || Number.isNaN(idJefeSuperior) || idJefeSuperior <= 0)) {
        mostrarNotificacion('warning', 'ID inválido', 'El ID de jefe superior no es válido.');
        return;
    }
    ...
    try {
        ...
        if (errores) {
            mostrarNotificacion('warning', 'Solicitudes con error', `${errores} solicitud(es) no pudieron guardarse. Revisa IDs y permisos.`);
        }
