...
def obtener_usuario_por_login(cursor, username_or_email: str):
    ...
    cursor.execute(
        "SELECT id_usuario_sistema, nombre_completo, email, PasswordHash, PasswordSalt, rol, estatus "
        "FROM dbo.tbl_usuarios_sistema "
        "WHERE email = ? OR nombre_completo = ?",
        username_or_email,
        username_or_email,
    )
...
def registrar_usuario(usuario: RegistroUsuario):
    ...
    try:
        ...
        cursor.execute(
            ...
            "(nombre_completo, email, PasswordHash, PasswordSalt, rol, id_usuario_original, estatus, password) "
            "VALUES (?, ?, ?, ?, ?, ?, 1, '')",
            usuario.nombre_usuario,
            usuario.email.lower(),
            hash_bytes,
...
def crear_usuario_sistema(
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO dbo.tbl_usuarios_sistema (nombre_completo, email, PasswordHash, PasswordSalt, rol, id_usuario_original, estatus, password) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, '')",
            usuario.nombre_usuario,
            usuario.email.lower(),
            hash_bytes,
